...
(async () => {
  const { data: pages, error } = await supabase.from('kassia_pages').select('id, slug, title');
  ...
  const { data: sections } = await supabase.from('kassia_page_sections').select('id, page_id, section_type, content, heading').in('page_id', pageIds);
  ...
  const inventory = matchedPages.map(page => {
    ...
    const hasPricingPreview = contentStr.includes('preturi') || contentStr.includes('pricing') || pageSections.some(s => s.section_type === 'pricing');
    ...
    const hasZoneLocale = contentStr.includes('zon') || contentStr.includes('cartier') || contentStr.includes('acoperi');
