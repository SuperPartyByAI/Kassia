  ...
  <a href="https://wa.me/40763795919?text=Buna!%20Vreau%20detalii%20despre%20spectacolul%20de%20magie." class="btn-primary" style="background:#25D366; color:white; padding:16px 32px; border-radius:99px; text-decoration:none; font-weight:800; font-size:1.1rem; box-shadow:0 10px 15px -3px rgba(37,211,102,0.4);">Scrie-ne pe WhatsApp</a>
    ...
    },
    {
      ...
      content: {
        ...
        body: '<p style="font-size:1.15rem; line-height:1.7;">Spectacolul nostru de magie este conceput special pentru dinamica petrecerilor de copii. Magicianul Kassia Events nu urcă pe o scenă îndepărtată pentru a face trucuri plictisitoare cu cărți, ci coboară direct în mijlocul copiilor, creând o energie debordantă.</p><p style="font-size:1.15rem; line-height:1.7;">Folosim recuzită colorată, elemente vizuale de impact și un stil de comunicare extrem de amuzant, adaptat pentru a menține vie atenția celor mici pe tot parcursul programului. Sărbătoritul este în centrul atenției, primind bagheta magică și rostind cuvintele fermecate care fac trucul să funcționeze!</p>'
      }
    },
    // Feature Cards (inserted right after the first generic section to trigger the CSS grid)
    {
      page_id: page.id,
      section_type: 'feature_card',
      order_index: 30,
      heading: 'Trucuri Interactive',
      content: {
        body: 'Fiecare copil are ocazia să se implice. Folosim baghete uriașe, cărți colorate uriașe și pălării fermecate.',
        image_url: '/images/animatori/jocuri-interactive-petreceri-copii.jpg',
        cta_text: 'Cere detalii',
        cta_url: 'https://wa.me/40763795919'
      }
    },
    {
      page_id: page.id,
      section_type: 'feature_card',
      order_index: 31,
      heading: 'Apariții Surpriză',
      content: {
        body: 'Porumbei sau un iepuraș adevărat pot apărea din cutii goale sau din pălăria magicianului, spre uimirea celor mici (disponibil în pachetele VIP).',
        image_url: '/images/animatori/baloane-modelabile-petreceri-copii.jpg',
        cta_text: 'Cere detalii',
        cta_url: 'https://wa.me/40763795919'
      }
    },
    {
      ...
      section_type: 'feature_card',
      order_index: 32,
      heading: 'Sărbătoritul e Vedeta',
      content: {
        body: 'Momentul de glorie al copilului tău! Va primi un rol principal și va ajuta la finalizarea marelui truc de la finalul spectacolului.',
        image_url: '/images/animatori/animator-pictura-pe-fata-copii.jpg',
        cta_text: 'Cere detalii',
        cta_url: 'https://wa.me/40763795919'
      }
    },
  ...
  const faqs = [
    {
      ...
      answer: "Spectacolul nostru este excelent pentru copiii între 3 și 12 ani. Pachetul Magic Show de 30 de minute este ideal pentru vârstele 3-6 ani (deoarece capacitatea lor de concentrare este mai mică, show-ul este mai alert și axat pe comedie). Pachetul VIP de 45 de minute este perfect pentru copiii de peste 6 ani care înțeleg concepte complexe și sunt fascinați de cum funcționează iluzionismul.",
      order_index: 10
    },
    {
      page_id: page.id,
      question: "Cum rezerv un magician și cu cât timp înainte?",
      answer: "Recomandăm rezervarea cu cel puțin 2-3 săptămâni înainte, mai ales dacă evenimentul are loc în weekend (sâmbătă sau duminică). Rezervarea se face foarte simplu prin WhatsApp la 0763 795 919 sau telefonic. Vom avea nevoie de locația exactă (București sau Ilfov), ora dorită și numărul aproximativ de copii.",
      order_index: 20
    },
    {
      page_id: page.id,
      question: "Apar animale vii în spectacolul de magie?",
      answer: "Da! Pachetul Iluzionism VIP poate include (în funcție de disponibilitatea artistului pe zona respectivă) apariția surpriză a unui iepuraș drăgălaș sau a porumbeilor. Acesta este mereu momentul de maximă uimire pentru cei mici. Vă rugăm să cereți detalii la confirmarea pachetului.",
      order_index: 30
    },
    {
      page_id: page.id,
      question: "Putem chema magicianul și la grădiniță?",
      answer: "Absolut! Deplasăm magicienii la serbări, petreceri de sfârșit de an școlar, sau aniversări celebrate la grădiniță sau școală. Ne adaptăm spațiului disponibil, chiar și în sălile de clasă.",
      order_index: 40
    },
    {
      page_id: page.id,
      question: "Există costuri de transport?",
      answer: "Pentru evenimentele desfășurate în București, transportul este de regulă inclus în prețul comunicat la ofertare. Pentru localitățile din Ilfov (sau mai îndepărtate), se poate percepe o mică taxă de transport în funcție de distanță. Vă vom comunica totul transparent din prima clipă.",
      order_index: 50
    }
  ];

  await supabase.from('kassia_faqs').insert(faqs);

  // 3. Add images to the gallery for this page
  // We'll map existing animatori generic images to this gallery just to populate it
  await supabase.from('kassia_gallery_items').delete().eq('page_id', page.id);
  const galleryItems = [
    { page_id: page.id, url: '/images/animatori/jocuri-interactive-petreceri-copii.jpg', alt_text: 'Jocuri copii magie', order_index: 10 },
    { page_id: page.id, url: '/images/animatori/animator-pictura-pe-fata-copii.jpg', alt_text: 'Bucurie copii magie', order_index: 20 },
    { page_id: page.id, url: '/images/animatori/baloane-modelabile-petreceri-copii.jpg', alt_text: 'Surpriza magie copii', order_index: 30 }
  ];
  await supabase.from('kassia_gallery_items').insert(galleryItems);

  console.log('GOD TIER Magic Page successfully built!');
}
