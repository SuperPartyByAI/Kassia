...
// Start by ensuring panel is open
let initialCheck = runJsInIframe("return doc.body.innerText;");
if (initialCheck === 'NO_IFRAME') {
    openPanel();
} else {
    // If we are in edit mode for a specific service, go back
    runJsInIframe(`
        let btns = Array.from(doc.querySelectorAll('button, div[role=button]'));
...
for (let srv of customServices) {
    ...
    // 1. Ensure "Adaugă un serviciu personalizat" is visible
    runJsInIframe(`
...
if (addedInCurrentBatch > 0) {
    ...
    savePanel();
    openPanel(); // reopen final for verification screenshot
}
