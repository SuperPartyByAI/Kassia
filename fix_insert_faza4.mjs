...
<div class="weddings-baptisms-block" style="background: #f8fafc; padding: 2rem; border-radius: 16px; margin-bottom: 3rem;">
    ...
    await supabase.from('kassia_page_sections').delete().eq('page_id', page.id).eq('heading', 'Test'); // clear the test item
    
    // I will use integer indexes:
    // order_index 5 (before 10 'Alege personajul')
    await supabase.from('kassia_page_sections').insert({
        page_id: page.id,
        section_type: 'custom_html',
        heading: '', // leaving empty to avoid duplicate rendering
        content: { is_active: true, body: vipBlockHTML },
        order_index: 5
    });

    // Upsell: order_index 83 (after 82 'Ce program alegi în funcție de vârsta copiilor?')
    await supabase.from('kassia_page_sections').insert({
        page_id: page.id,
        section_type: 'custom_html',
        heading: '', 
    ...
    console.log("Inserted using INTEGER indexes.");
}
