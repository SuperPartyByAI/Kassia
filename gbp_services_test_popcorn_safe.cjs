...
// 2. Inside iframe, click "Adauga un serviciu personalizat" (maybe under "Adaugati mai multe servicii" first)
let addCustomJs = `
tell application "Google Chrome"
    execute active tab of first window javascript "
    (function() {
...
`;
console.log("Prepare custom btn: " + runAppleScript(addCustomJs));
execSync('sleep 2');
...
if (val.trim() === srv) {
    ...
    `;
    console.log("Save status: " + runAppleScript(saveJs));
    execSync('sleep 5');
