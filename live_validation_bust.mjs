...
async function validate() {
  ...
  const res = await fetch(url, { headers: { 'Cache-Control': 'no-cache, no-store, must-revalidate', 'Pragma': 'no-cache' }});
  ...
  const terms = {
      ...
      'Un personaj animator sau două': /Un personaj animator sau două/i.test(text),
