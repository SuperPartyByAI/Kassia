...
async function run() {
  console.log("=== RUNNING BACKUP BEFORE DB WRITE ===");
  
  const { data: pages, error: pagesErr } = await supabase
    .from('kassia_pages')
    .select('id, path')
    .in('path', pathsToBackup);
  ...
  fs.writeFileSync('db_backup_pre_personaje.json', JSON.stringify(backupData, null, 2));
  console.log(`Backup saved to db_backup_pre_personaje.json with ${sections.length} sections from ${pages.length} pages.`);

  const rollbackScript = `import { createClient } from '@supabase/supabase-js';
import dotenv from 'dotenv';
import fs from 'fs';
...
`;
  ...
  fs.writeFileSync('rollback_personaje.mjs', rollbackScript);
  console.log("Rollback script generated: rollback_personaje.mjs");
}
