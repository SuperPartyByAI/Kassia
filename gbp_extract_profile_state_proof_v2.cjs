...
(async function() {
    ...
    let result = {
      source_context: "GOOGLE_SERP_ONLY",
    ...
    let keywords = [
        "GBP_NMX", "GBP_LOCATIONS", "GOOGLE_SERP_ONLY",
    ...
    try {
        ...
        if (hasNMX) {
            result.source_context = "GBP_NMX";
            result.pass_checks.is_gbp_panel = true;
        } else if (window.location.href.includes('business.google.com/locations')) {
            result.source_context = "GBP_LOCATIONS";
        }
        ...
        if (!hasNMX && !window.location.href.includes('business.google.com/locations')) {
            result.hold_reasons.push("Nu am putut intra în panoul Google Business Profile / NMX. Am găsit doar SERP sau pagină fără panou editabil.");
        ...
        if (!opened) {
            if (result.source_context === 'GBP_NMX') {
               result.hold_reasons.push("Deși suntem pe NMX, nu am putut deschide panoul de editare profil.");
            } else {
               result.hold_reasons.push("Nu am putut deschide Editează profilul.");
            }
            window.extractedGBPResult = JSON.stringify(result, null, 2);
            return;
        }
        ...
        if (!all24) result.hold_reasons.push("Programul nu este 24 de ore în fiecare zi.");
        ...
        if (servOpened) {
            ...
            if (!hasAll) {
                result.pass_checks.custom_services_pending = true;
                result.hold_reasons.push("Serviciile custom obligatorii nu sunt adăugate încă.");
            }
...
tell application "Google Chrome"
    ...
    -- 1. Check if we already have a tab with NMX (Compania ta pe Google / Editează profilul)
    repeat with w in windows
        repeat with t in tabs of w
            ...
            if u contains "business.google.com" and title of t contains "Kassia" then
                set the_tab to t
                set the_win to w
                exit repeat
            end if
    ...
    -- 2. If no NMX tab, check for business.google.com/locations
    ...
    -- Bring to front (just bring window to front)
    set index of the_win to 1
    
    -- If we are on locations, click Kassia Events
    ...
    -- 4. Inject the extraction script
    execute the_tab javascript "${jsSnippet}"
    
    set wait_time to 0
    repeat while wait_time < 30
        delay 1
        set wait_time to wait_time + 1
        set res to execute the_tab javascript "window.extractedGBPResult"
...
fs.writeFileSync('/tmp/runner2.scpt', appleScript);
...
let fallbackJson = {
    ...
    hold_reasons: ["Nu am putut intra în panoul Google Business Profile / NMX. Am găsit doar SERP sau pagină fără panou editabil."]
};

try {
    const out = execSync('osascript /tmp/runner2.scpt', { encoding: 'utf8' }).trim();
    if (out.startsWith("ERROR:")) {
        console.error(out);
        fallbackJson.hold_reasons.push(out);
        fs.writeFileSync('/tmp/gbp_profile_state_proof.json', JSON.stringify(fallbackJson, null, 2));
    } else {
