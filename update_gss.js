...
const servicesReplacement = `const services = [
    ...
    { title: 'Buchete din Baloane', img: 'buchete.png', url: '/buchete-baloane-heliu-bucuresti/', desc: 'Grupaje elegante din baloane cu heliu, ideale pentru cadou.', kw: 'buchete grupaj cadou surpriza greutate buchet baloanecadou' },
    ...
    { title: 'Absolvire & Școală', img: 'absolvire.png', url: '/decoratiuni-baloane-absolvire-scoli-bucuresti/', desc: 'Cadrul perfect pentru o nouă etapă.', kw: 'absolvire scoala gradinita banchet clasa promotia serbare sfarsit de an festivitati clasa a 8 a' }
];`;

content = content.replace(/const services = \[[\s\S]*?\];/, servicesReplacement);

const searchContainerHTML = `<div class="gss-search-container">
        ...
        <input type="text" id="gss-search-input" placeholder="Caută serviciul (ex: vata, magic, elsa...)" autocomplete="off" autofocus />
        <button id="gss-voice-btn" class="gss-voice-btn" aria-label="Căutare Vocală" title="Caută vocal">
          <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M12 2a3 3 0 0 0-3 3v7a3 3 0 0 0 6 0V5a3 3 0 0 0-3-3Z"></path><path d="M19 10v2a7 7 0 0 1-14 0v-2"></path><line x1="12" y1="19" x2="12" y2="22"></line></svg>
        </button>
      </div>
      <div id="gss-voice-status" class="gss-voice-status" style="display:none; color: #3b82f6; font-size: 0.9rem; font-weight: 600; padding-top: 0.5rem; text-align: center;"></div>`;

content = content.replace(/<div class="gss-search-container">[\s\S]*?<\/div>/, searchContainerHTML);
...
const styleAdd = `
  ...
  #gss-no-results p.fallback-msg {
    color: #3b82f6;
    margin-top: 0.5rem;
    font-size: 1.1rem;
  }
`;
content = content.replace('</style>', styleAdd + '\n</style>');

content = content.replace(/<a href=\{svc\.url\} class="gss-card" data-search=\{.*?\}\>/g, '<a href={svc.url} class="gss-card" data-title={svc.title} data-desc={svc.desc} data-kw={svc.kw}>');
      ...
      cards.forEach((card) => {
        ...
        if (queryWords.length === 0) {
           ...
           queryWords.forEach(qw => {
               ...
               if (!wordMatched) allWordsMatched = false;
           });
