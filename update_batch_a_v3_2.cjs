...
const logFile = path.join(__dirname, 'batch_a_v3_2_run.log');
function log(msg) {
...
const forbiddenStrings = [
  ...
  "vată de zahăr", "fotograf", "magie", "Placeholder", "Distracție garantată", 
  "Absolut", "excelentă", "engleză"
];
...
const restaurantData = {
  ...
  faqs: [
    ...
    { id: '0f37187b-fe52-4e60-8fac-f873ae617d32', question: "Cum gestionăm muzica într-un restaurant?", answer: "<p>Volumul se adaptează regulilor locației. Putem folosi muzică de fundal pentru momentele de joc și dans, fără să transformăm programul într-un eveniment zgomotos.</p>" },
...
const corporateData = {
  ...
  faqs: [
    ...
    { id: '45e0d028-8112-43b6-8f64-44e25d8406ac', question: "Putem organiza activitățile copiilor într-o sală de meeting separată?", answer: "<p>Da, activitățile pot fi organizate într-o sală separată dacă locația permite. Această variantă ajută la delimitarea zonei copiilor de zona principală a evenimentului.</p>" },
    { id: '2850f4d0-3b86-4df8-b324-e9613f5c2ef7', question: "Putem adapta comunicarea dacă sunt copii care nu vorbesc română?", answer: "<p>Spune-ne din timp componența grupului. În funcție de echipa disponibilă și de vârstele copiilor, putem propune activități simple, ușor de urmărit vizual.</p>" }
  ]
};

// Exclude karaoke question manually from corporateData checks as "karaoke" was never forbidden but the prompt asked to remove it from restaurant, and there is no english promise now.
// I already replaced the karaoke string in restaurant faq #8. Oh wait, I see I missed karaoke in restaurant FAQ #8! Let's fix that.
restaurantData.faqs[7] = { id: '2ee95be9-d4cf-4f90-8936-2634a870c646', question: "Ce facem cu volumul pentru a nu deranja?", answer: "<p>Evităm volumul puternic în spații închise pentru a menține un ambient plăcut, concentrându-ne pe jocuri interactive și ateliere creative la masă.</p>" };
...
async function dbUpdate(table, pageId, idField, idValue, payload) {
  const res = await fetch(`${supabaseUrl}/rest/v1/${table}?${idField}=eq.${idValue}&page_id=eq.${pageId}`, {
    method: 'PATCH',
    headers: {
      'apikey': supabaseKey, 
...
async function dbUpdatePage(table, pageId, payload) {
  const res = await fetch(`${supabaseUrl}/rest/v1/${table}?id=eq.${pageId}`, {
    method: 'PATCH',
    headers: {
      'apikey': supabaseKey, 
      'Authorization': `Bearer ${supabaseKey}`,
...
async function runUpdate() {
  log("Starting Batch A V3.2 Validation & Update process via REST...");
  ...
  for (const t of targets) {
    ...
    if (!DRY_RUN) {
      await dbUpdatePage('kassia_pages', t.page_id, t.updates);
    }
    ...
    for (const s of t.sections) {
      ...
      checkForbidden(newContent, `Merged section content ${s.id}`);

      log(`SECTION UPDATE: ${s.id} (BEFORE: ${existS.heading}) -> (AFTER: ${s.heading}) (Subheading is ${newContent.subheading === null ? 'null' : newContent.subheading})`);
      if (!DRY_RUN) {
        await dbUpdate('kassia_page_sections', t.page_id, 'id', s.id, { heading: s.heading, content: newContent, updated_at: nowIso });
      }
    }
    ...
    for (const f of t.faqs) {
      ...
      checkForbidden(f, `FAQ Payload ${f.id}`);

      log(`FAQ UPDATE: ${f.id} (BEFORE: ${existF.question}) -> (AFTER: ${f.question})`);
      if (!DRY_RUN) {
        await dbUpdate('kassia_faqs', t.page_id, 'id', f.id, { question: f.question, answer: f.answer, updated_at: nowIso });
      }
    }
  }
