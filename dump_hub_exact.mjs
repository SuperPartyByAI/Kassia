...
(async () => {
  ...
  const page = pages[0];
  const { data: sections } = await supabase.from('kassia_page_sections').select('id, section_type, title, heading, order_index, content').eq('page_id', page.id).order('order_index');
