...
<div class="kassia-ecosystem" style="background: #ffffff; padding: 4rem 2rem; border-radius: 24px; box-shadow: 0 20px 40px rgba(0,0,0,0.04); margin-bottom: 4rem; border: 1px solid #f1f5f9;">
    ...
    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 2rem;">
        ...
        <div style="background: #f8fafc; border-radius: 16px; padding: 2rem; border: 1px solid #e2e8f0; transition: transform 0.2s;">
            ...
            <ul style="list-style: none; padding: 0; margin: 0; display: flex; flex-direction: column; gap: 1rem;">
                ...
                <li style="display: flex; align-items: flex-start; gap: 0.75rem;"><span style="color: #10b981; font-weight: bold; font-size: 1.2rem;">✓</span> <div><strong style="color:#0f172a;">Baloane Uriașe (Gigantice) de Săpun</strong><br><span style="font-size: 0.95rem; color: #475569;">Momente magice vizuale perfecte pentru fotografii de vis.</span></div></li>
            </ul>
        </div>

        <!-- Sărbători Sezoniere (NOU) -->
        <div style="background: #f8fafc; border-radius: 16px; padding: 2rem; border: 1px solid #e2e8f0; transition: transform 0.2s;">
    ...
    await supabase.from('kassia_page_sections').update({
        content: { is_active: true, body: finalHubHTML }
    }).eq('page_id', page.id).eq('order_index', 83);

    console.log("Updated order_index 83 with the COMPREHENSIVE MEGA HUB of services.");
}
run();
