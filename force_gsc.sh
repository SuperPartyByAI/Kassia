...
echo "Looking for inspection bar and injecting Homepage URL..."
run_js "
...
if [[ "$res" == *"Clicked"* ]]; then
    ...
    for i in {1..30}; do
        ...
        if [[ "$modal" == *"Indexarea a fost solicitată"* ]] || [[ "$modal" == *"Indexing requested"* ]]; then
            echo "Modal found!"
            osascript -e 'tell application "Google Chrome" to activate'
            sleep 1
            screencapture -x /Users/universparty/.gemini/antigravity/brain/6d86611f-e938-48db-98c9-3bb7d0b0f227/homepage_real_indexing_proof.png
    ...
    echo "Could not click the button. Maybe the URL is already in queue or loading failed."
fi
