...
(async () => {
  ...
  const { data: pages } = await supabase.from('kassia_pages').select('id, slug, title, is_active').in('slug', targetSlugs);
  ...
  const inventory = pages.map(page => {
    ...
    // Analyze sections to see if they have the standard blocks
    const contentStr = JSON.stringify(pageSections).toLowerCase();
    
    const hasPricingPreview = contentStr.includes('preturi') || contentStr.includes('pricing'); // Rough check
    ...
    const hasUnSauDoua = contentStr.includes('un personaj') && contentStr.includes('doua') || contentStr.includes('două');
    const hasFAQ = pageSections.filter(s => s.section_type === 'faq').length;
